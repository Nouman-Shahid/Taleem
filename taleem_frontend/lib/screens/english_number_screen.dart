...
class _EnglishNumberScreenState extends State<EnglishNumberScreen> {
  ...
  Future<void> _play(Lesson l) async {
    ...
    await TtsService.speak(l.translation ?? l.displayText, languageCode: 'en');
    if (mounted) setState(() => _speakingId = null);
  }

  Future<void> _load() async {
    try {
      final list = await LessonService.listByType('english_number');
      if (!mounted) return;
      setState(() {
        _lessons = list;
        _loading = false;
